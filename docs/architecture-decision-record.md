...
### 7.3. Concentric radius rule

Когда контейнер собирается **из компонентов** (Dialog с кнопками,
Dropdown с MenuItem'ами, Card с инпутами и т.д.), его radius должен
быть **на один шаг больше** размера вложенных элементов. Это даёт
визуально-концентрические rounded corners — внутренние элементы
"сидят" в контейнере, а не торчат углами в его углы.

**Forward правило** (контейнер на основе компонентов):

| Размер вложенных | Token контейнера | Value |
|---|---|---|
| Components `xs` (radius 8px)  | `radius.comp.sm` | 10px |
| Components `sm` (radius 10px) | `radius.comp.md` | 12px |
| Components `md` (radius 12px) | `radius.comp.lg` | 14px |
| Components `lg` (radius 14px) | `radius.comp.xl` | 16px |
| Components `xl` (radius 16px) | `radius.comp.xl` | 16px (cap) |

Базовые случаи (с дефолтом MD внутри):
- **Dialog / Drawer** → `radius.comp.lg` (14px)
- **DropdownMenu / Popover / NotificationCenter** → `radius.comp.lg` (14px)
- **Card** — если внутри только MD-инпуты → `radius.comp.lg`; если
  внутри LG-формы → `radius.comp.xl`
- **Toast** — обычно содержит SM-кнопки → `radius.comp.md` (12px)

**Обратное правило** (вложенный компонент в контейнер фиксированного
размера):
...
Применяется когда внутренний элемент **жёстко прилегает** к границе
контейнера (e.g. таб внутри тёмной полосы header'а). Если между
внутренним элементом и границей есть padding (что обычно так и есть
у Dialog / Dropdown / Card) — используй forward-правило выше.

**Глубина вложенности** — максимум 2 уровня. Dialog → Card →
Button-внутри-Card: первая ступенька (MD → LG) даёт хорошую
концентричность, вторая (LG → XL на Card) ещё ок, третья
исчерпывает radius-ramp. На третьем уровне держим Card на том же
LG что и Dialog.

Это правило фиксируется в коде (token bindings компонентов), а не
оставляется на усмотрение разработчика.
