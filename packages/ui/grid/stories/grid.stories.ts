...
const cell = (n: number) => `<div style="background:#E4E4E7;border-radius:6px;height:80px;display:flex;align-items:center;justify-content:center;color: var(--kp-color-gray-600);font-size:12px;font-family:Onest,system-ui,sans-serif">${n}</div>`;

export const Columns: Story = {
  render: () => ({
    template: `
      <div style="display:flex;flex-direction:column;gap:24px;font-family:Onest,system-ui,sans-serif;font-size:12px;color: var(--kp-color-gray-600)">
        ${[2,3,4,6,12].map(c => `
          <div>
      ...
      <kp-grid [columns]="3" gap="md">
        ${[1,2,3,4,5,6].map(n => `
          <div style="background: var(--kp-color-white);border: 1px solid var(--kp-color-gray-200);border-radius:8px;padding:16px;font-family:Onest,system-ui,sans-serif">
            ...
            <div style="font-size:13px;color: var(--kp-color-gray-600);margin-top:4px">$${n * 24}.00</div>
          </div>
        `).join('')}
      </kp-grid>
    `,
